    ...
    const obtenerPartidos = async () => {
      try {
        const res = await axios.get(
          `${api}/api/partidos/todos/portorneo/${torneoId}`
        );
    ...
    <div className="contenedor-arbitro">
      ...
      <div className="filtros">
        ...
        <button
          ...
          className={`filtro-btn ${filtro === "JUGADOS" ? "activo" : ""}`}
        >
          JUGADOS
        </button>
      </div>
          ...
          partidosPagina.map((p) => (
            ...
            >
              ...
              <p style={{ fontWeight: "bold" }}>
                {p.equipoLocal.nombreEquipo} vs {p.equipoVisitante.nombreEquipo}
              </p>
              <p>
                📅 {p.fechaPartido} 🕒 {p.hora}
              </p>
              <p style={{ fontWeight: "bold", color: "#666" }}>
                {p.tipoPartido === "JORNADA REGULAR"
                  ? "Jornada Regular"
                  : p.tipoPartido}
              </p>
              <button onClick={() => abrirModal(p)} className="btn btn-danger">
                DETALLES
              </button>
            </div>
          ))
      ...
      {modalAbierto && partidoSeleccionado && (
        ...
        >
          <div
            ...
            className="quitarScroll"
          >
            <h4 style={{ textAlign: "center" }}>
              {partidoSeleccionado.torneo?.nombreTorneo}
            </h4>
            <h5 style={{ textAlign: "center" }}>
              {partidoSeleccionado.equipoLocal.nombreEquipo} vs{" "}
              {partidoSeleccionado.equipoVisitante.nombreEquipo}
            </h5>
            <p style={{ textAlign: "center" }}>
              📅 {partidoSeleccionado.fechaPartido} 🕒{" "}
              {partidoSeleccionado.hora}
            </p>
            <p>
              <strong>Campo:</strong> {partidoSeleccionado.cancha.campo.nombre}
            </p>
            <p>
              <strong>Dirección:</strong>{" "}
              {partidoSeleccionado.cancha.campo.direccion}
            </p>
            <iframe
            ...
            {partidoSeleccionado.jugado && (
              <>
                <p>
                  <strong>Goles Local:</strong> {partidoSeleccionado.golesLocal}
                </p>
                <p>
                ...
                {partidoSeleccionado.tipoDesempate === "PENALES" && (
                  <>
                    <p>
                      <strong>Penales Local:</strong>{" "}
                      {partidoSeleccionado.golesLocalPenales}
                    </p>
                    <p>
                      <strong>Penales Visitante:</strong>{" "}
                      {partidoSeleccionado.golesVisitantePenales}
                    </p>
                  </>
                )}
              </>
